...
object logMacro extends MacroCommon {
  def impl(c: whitebox.Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
    ...
    val args: (Boolean, LogType) = extractArgumentsTuple2(c) {
      case q"new log(logType=$logType)" => (false, evalTree(c)(logType.asInstanceOf[Tree]))
      case q"new log(verbose=$verbose)" => (evalTree(c)(verbose.asInstanceOf[Tree]), LogType.JLog)
      case q"new log($logType)" => (false, evalTree(c)(logType.asInstanceOf[Tree]))
      case q"new log(verbose=$verbose, logType=$logType)" => (evalTree(c)(verbose.asInstanceOf[Tree]), evalTree(c)(logType.asInstanceOf[Tree]))
